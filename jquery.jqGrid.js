...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		if (ts.p.colModel.length > 0) {
		    var items = {};

		    $.each(ts.p.colModel, function (index, value) {
		        var item = {};
		        ...
		        if (value.visibility) {
		            if (value.visibility == "fixed") {
		                item.disabled = true;
		                items[value.name] = item;
		            }
		    ...
		    $.contextMenu({
		        selector: '#gbox_' + $.jgrid.jqID(ts.p.id) + ' > div .ui-jqgrid-hdiv:first',
		        className: $.jgrid.jqID(ts.p.id) + "_menu",
		        trigger: 'right',
		        delay: 500,
		        items: items,
		        events: {
		    ...
		    var g = $(this);
		    $("." + $.jgrid.jqID(ts.p.id) + "_menu" + " > .context-menu-item input[type=checkbox]").change(function () {
		        var value = $(this).is(':checked');
	
		        var valueSplit = $(this).attr("name").split("context-menu-input-");

		        if (value) {
		            $(g).showCol(valueSplit[1]);
		        } else {
		            $(g).hideCol(valueSplit[1]);
		        }
		    });
		}
